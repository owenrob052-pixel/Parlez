...
const app = {
    ...
    init() {
        ...
        document.getElementById('btn-review').style.display = weakCount > 0 ? 'flex' : 'none';
        if (weakCount > 0) {
            document.getElementById('btn-review').querySelector('.mode-desc').textContent =
    ...
    initTTS() {
        const loadVoices = () => {
            ...
            // Prefer a female French voice
            const frenchVoices = voices.filter(v => v.lang.startsWith('fr'));
            // Try to find a female voice (common names: Amélie, Virginie, Marie, Léa)
            const femaleKeywords = ['female', 'amélie', 'amelie', 'virginie', 'marie', 'léa', 'lea', 'céline', 'celine', 'audrey', 'cloe', 'siri'];
            let picked = frenchVoices.find(v => femaleKeywords.some(k => v.name.toLowerCase().includes(k)));
            if (!picked) picked = frenchVoices[0]; // fallback to any French voice
            this.frenchVoice = picked || null;
            if (this.frenchVoice) {
                console.log('TTS voice:', this.frenchVoice.name, this.frenchVoice.lang);
            } else {
                console.warn('No French TTS voice found. Install French language pack for TTS.');
            }
        };
    ...
    startSession(mode) {
        ...
        if (mode === 'sprint') {
            ...
            this.buildQueue(this.getWeakWords());
        } else if (mode === 'typing') {
    ...
    getNewWords(count) {
        ...
        for (let i = 0; i < count; i++) {
            // 15% chance to double-check a mastered word
            if (mastered.length > 0 && Math.random() < 0.15) {
                const r = Math.floor(Math.random() * mastered.length);
                selection.push(mastered[r]);
            } else {
                // Find the next most common unseen word
    ...
    checkTyped() {
        ...
        // Normalize: lowercase, strip accents, strip non-alpha except spaces
        const normalize = s => s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z ]/g, '').replace(/\s+/g, ' ').trim();
        const stripParens = s => s.replace(/\s*\([^)]*\)/g, '').trim();
    ...
    recordResult(isCorrect, correctAnswer) {
        ...
        if (!ex.isSentence) {
            ...
            if (isCorrect) {
                p.correct++;
                // Require 25 correct answers and 85% accuracy to master
                if (p.correct >= 25 && p.correct / (p.correct + p.incorrect) >= 0.85) {
                    p.mastered = true;
                }
            } else {
                p.incorrect++;
